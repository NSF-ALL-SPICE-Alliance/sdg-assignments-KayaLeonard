...
```{r}
ggplot(data = sdr_data, aes(x= goal_10_score, y= goal_11_score, color = goal_12_score)) +
  geom_point()
```
There is a wide spread shown in this graph, without a clear trend between goals 10 and 11, reduced inequalities and sustainable cities and communities respectively. However with goal 12, responsible consumption and production, lower scores appear to correspond to high goal 10 and 11 scores, with the darkest blue points in the top right corner. Lower goal 11 scores appear to correspond to slightly higher goal 12 scores, with the goal 12 scores decreasing as the goal 11 scores increase. There does not appear to be a clear relationship between goal 10 score and goal 12 score.
...
```{r}
ggplot(mena_sdr_data, aes(x= goal_13_score, y= country)) +
  geom_bar(stat = "identity")
```
...
Countries in the high (80-90) score range for economic growth are largely OECD countries, wih the exception of a LAC outlier in the highest score region shown. Countries in the low 40-60 score range are primarily Sub-Saharan African countries, with a notable oulier of an E. Europe & C. Asian country as the lowest score shown depsite other countries from that region performing largely in the 65-90 range.