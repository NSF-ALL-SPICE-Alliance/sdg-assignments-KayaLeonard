...
```{r}
goal_3_goal_4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(Title= "Goal 3 score vs. Goal 4 score", x= "SDG 3 score", y= "SDG 4 score",color= "Region")
ggplotly(goal_3_goal_4_scatterplot)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("BuPu", goal_5_score)(goal_5_score), label = mytext)
```
